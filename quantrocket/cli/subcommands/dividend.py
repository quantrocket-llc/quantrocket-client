...
def add_subparser(subparsers):
    ...
    group.add_argument("-x", "--proximity", default=20, type=int, metavar="DAYS", help="consider identical dividend amounts fewer than this many days apart to be duplicates")
    group.add_argument("-f", "--fuzziness", default=0.05, type=float, metavar="FLOAT", help="consider clustered dividend amounts differing by less than this percentage to be duplicates")
    ...
    group.add_argument("--purge-minorities", dest="purge_minorities", action="store_true", help="purge clustered dividends that disagree with the majority opinion of the cluster")
    group.add_argument("--purge-vendors", dest="purge_vendors", type=parse_dict, nargs="*", metavar="PREF:UNPREF", help="purge clustered dividends by favoring a preferred over an unpreferred vendor")
    group.add_argument("--purge-duplicates", dest="purge_duplicates", action="store_true", help="purge clustered dividends by removing all but the first clustered dividend on a per-vendor basis")
    ...
    parser.add_argument("-g", "--groups", nargs="*", metavar="GROUP", help="limit to these groups")
    parser.add_argument("-i", "--conids", nargs="*", metavar="CONID", help="limit to these conids")
    parser.add_argument("-x", "--proximity", default=20, type=int, metavar="DAYS", help="consider identical dividend amounts fewer than this many days apart to be duplicates")
    parser.add_argument("-f", "--fuzziness", default=0.05, type=float, metavar="FLOAT", help="consider clustered dividend amounts differing by less than this percentage to be duplicates")
    ...
    parser.add_argument("-p", "--purge-minorities", dest="purge_minorities", action="store_true", help="purge clustered dividends that disagree with the majority opinion of the cluster")
    parser.add_argument("-v", "--purge-vendors", dest="purge_vendors", type=parse_dict, nargs="*", metavar="PREF:UNPREF", help="purge clustered dividends by favoring a preferred over an unpreferred vendor")
    parser.add_argument("-r", "--purge-duplicates", dest="purge_duplicates", action="store_true", help="purge clustered dividends by removing all but the first clustered dividend on a per-vendor basis")
