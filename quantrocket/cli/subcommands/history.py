...
Create an end-of-day database for US stocks:

    quantrocket history create-polygon-db 'usa-stk-1d' --universes 'usa-stk' --bar-size '1 day'

Create a 1-minute database for a universe of US stocks and shard by day:

    quantrocket history create-polygon-db 'usa-stk-1min' --universes 'usa-stk' --bar-size '1 minute' --shard 'day'
    """
    parser = _subparsers.add_parser(
        "create-polygon-db",
        help="create a new database for collecting historical data from Polygon",
        epilog=examples,
        formatter_class=argparse.RawDescriptionHelpFormatter)
    parser.add_argument(
        "code",
