...
Get a list of gateways that are running and have permission for NYSE market data
and Reuters research (multiple search criteria are ANDed together)

    quantrocket launchpad status --exchanges NYSE --research reuters --status running
    """
    parser = _subparsers.add_parser(
        "status", help=" query statuses of IB Gateway services", epilog=examples,
    ...
    parser.add_argument("-t", "--sec-type", dest="sec_type", choices=["STK", "FUT", "CASH", "OPT"], help="limit to IB Gateway services with market data permission for this securitiy type (useful for disambiguating permissions for exchanges that trade multiple asset classes)")
    parser.add_argument("-r", "--research", metavar="VENDOR", dest="research_vendors", nargs="*", choices=["reuters", "wsh"], help="limit to IB Gateway services with permission for these research vendors")
...
Restart all gateways:
    ...
    """
    parser = _subparsers.add_parser(
        "start", help="start one or more IB Gateway services", epilog=examples,
        formatter_class=argparse.RawDescriptionHelpFormatter)
    parser.add_argument("-e", "--exchanges", metavar="EXCHANGE", nargs="*", help="limit to IB Gateway services with market data permission for these exchanges")
    parser.add_argument("-t", "--sec-type", dest="sec_type", choices=["STK", "FUT", "CASH", "OPT"], help="limit to IB Gateway services with market data permission for this securitiy type (useful for disambiguating permissions for exchanges that trade multiple asset classes)")
    parser.add_argument("-r", "--research", metavar="VENDOR", dest="research_vendors", nargs="*", choices=["reuters", "wsh"], help="limit to IB Gateway services with permission for these research vendors")
    parser.add_argument("-g", "--gateways", metavar="SERVICE_NAME", nargs="*", help="limit to these IB Gateway services")
    parser.add_argument("-w", "--wait", action="store_true", default=False, help="wait for the IB Gateway services to start before returning (default is to start the gateways asynchronously)")
...
Stop gateways with Japan stock permissions:
    ...
    """
    parser = _subparsers.add_parser(
        "stop", help="stop one or more IB Gateway service", epilog=examples,
        formatter_class=argparse.RawDescriptionHelpFormatter)
    parser.add_argument("-e", "--exchanges", metavar="EXCHANGE", nargs="*", help="limit to IB Gateway services with market data permission for these exchanges")
    parser.add_argument("-t", "--sec-type", dest="sec_type", choices=["STK", "FUT", "CASH", "OPT"], help="limit to IB Gateway services with market data permission for this securitiy type (useful for disambiguating permissions for exchanges that trade multiple asset classes)")
    parser.add_argument("-r", "--research", metavar="VENDOR", dest="research_vendors", nargs="*", choices=["reuters", "wsh"], help="limit to IB Gateway services with permission for these research vendors")
    parser.add_argument("-g", "--gateways", metavar="SERVICE_NAME", nargs="*", help="limit to these IB Gateway services")
    parser.add_argument("-w", "--wait", action="store_true", default=False, help="wait for the IB Gateway services to stop before returning (default is to stop the gateways asynchronously)")
...
Show current config:
    ...
    """
    parser = _subparsers.add_parser(
        "config", help="upload a new config, or return the current configuration", epilog=examples,
        formatter_class=argparse.RawDescriptionHelpFormatter)
    parser.add_argument("filename", nargs="?", metavar="FILENAME", help="the config file to upload (if omitted, return the current config)")
